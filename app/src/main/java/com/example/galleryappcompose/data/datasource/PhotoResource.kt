...
class PhotoResource(private val apiService: ApiService): PagingSource<Int, PhotoResponse.Photo>() {
    override fun getRefreshKey(state: PagingState<Int, PhotoResponse.Photo>): Int? {
        return state.anchorPosition?.let { anchorPosition ->
            val anchorPage: LoadResult.Page<Int, PhotoResponse.Photo>? = state.closestPageToPosition(anchorPosition)
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, PhotoResponse.Photo> {
        return try {
            ...
            val response = apiService.getAllPhotos(curPage, params.loadSize)
            LoadResult.Page(
